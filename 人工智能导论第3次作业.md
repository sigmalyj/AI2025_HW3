...
graph TD
    A["s
    (batchsize × board_x × board_y)"] --> B["s.unsqueeze(1)
    (batchsize × 1 × board_x × board_y)"]
    B --> C["conv1 + ReLU
    (batchsize × num_channels × board_x × board_y)"]
    C --> D["reshape
    (batchsize × board_x*board_y*num_channels)"]
    D --> E["fc1 + ReLU
    (batchsize × linear_hidden[0])"]
    E --> F["fc2 + ReLU
    (batchsize × linear_hidden[1])"]
    F --> G["policy_head + log_softmax
    (batchsize × action_space_size)"]
    F --> H["value_head + tanh
    (batchsize × 1)"]
    ...
    style F fill:#4aa,stroke:#333
    style G fill:#4af,stroke:#333
